...
class Dubin:
    ...
    def get_shortest_path(self, car, target, car_current_pos, next_pos):
        ...
        for index, path in enumerate(path_types):
            ...
            prefix = 'c:F'
            message_one = prefix + chosen_path[0].upper() + str(int(round(alpha_one * 1000))).zfill(6)
            message_three = prefix + chosen_path[2].upper() + str(int(round(alpha_three * 1000))).zfill(6)
            
            if path_names[index][1] == 'S':
                length = details[1] / settings.grid_size * 10
                message_two = prefix + chosen_path[1].upper() + str(int((length * 1000))).zfill(6)
            else:
                alpha_two = 360 - abs(math.degrees(details[1]))
                message_two = prefix + chosen_path[1].upper() + str(int(round(alpha_two * 1000))).zfill(6)
            ...
            if 0 < alpha_one < 180:
                prefix = 'c:F'
                message_one = get_message(prefix, alpha_one, chosen_path[0])
                message_three = get_message(prefix, alpha_three, chosen_path[2])
            else:
                prefix = 'c:B'
            ...
            compiled_path_details.append([coordinates, time, chosen_path, [message_one, message_two, message_three]])
