...
while True:
    ...
    # next_pos[0] = path_taken[0][0]
    # next_pos[1] = path_taken[0][1]
    # next_pos[2] = path_taken[0][2]
    ...
    if (destinations):
        message_coord = str(obstacles_grid[tuple(path_taken[0])][0]) + "," + str(abs(20 - obstacles_grid[tuple(path_taken[0])][1]))
        message_list.extend(["c:END00000", "c:TAKEPIC" + message_coord])
        send_message = path_found = True
        no_path = False
        car_current_pos = destinations[-1]
        break
    
    else:
        path = path_taken.pop(0)
        path_taken.append(path)

    # END OF A* SEARCH -----------------------------------------------------------------------------------------
    # end of find first path
