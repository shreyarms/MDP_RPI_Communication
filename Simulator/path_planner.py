from Simulator.Entities.grid import *
from Simulator.Entities.car import *
from Simulator.Entities.obstacle import *
import algorithm
import a_star
import settings
...
class path_planner:
    ...
    # find first path
        # DUBINS path planning -----------------------------------------------------------------------------------
            ...
            if (destinations):
                message_coord = str(obstacles_grid[tuple(path_taken)][0]) + "," + str(abs(20 - obstacles_grid[tuple(path_taken)][1]))
    ...
    def find_invalid(self,obs):
        ...
        for obstacle in obs.obstacle_coordinates:
            bottom_left_x = round(obstacle[0] - OBSTACLE_CUSHION + 1)
            bottom_left_y = round(obstacle[1]  + OBSTACLE_CUSHION - 1)

            top_right_x = round(obstacle[0]  + OBSTACLE_CUSHION + settings.grid_size - 2)
            top_right_y = round(obstacle[1]  - OBSTACLE_CUSHION - settings.grid_size + 2)
